class GroceryData{

    // List of maps representing grocery products
    static List<Map<String, dynamic>> groceryProducts = [
      {
        'id': '1',
        'name': 'Apple',
        'description': 'Fresh and juicy red apples packed with nutrients.',
        'price': 1.2,
        'image': 'https://5.imimg.com/data5/AK/RA/MY-68428614/apple.jpg',
      },
      {
        'id': '2',
        'name': 'Banana',
        'description': 'Sweet and ripe bananas, perfect for snacking.',
        'price': 0.8,
        'image': 'https://images.immediate.co.uk/production/volatile/sites/30/2017/01/Bunch-of-bananas-67e91d5.jpg',
      },
      {
        'id': '3',
        'name': 'Carrot',
        'description': 'Crunchy and fresh carrots rich in vitamins.',
        'price': 1.0,
        'image': 'https://exat8rt6fi5.exactdn.com/wp-content/uploads/2022/05/carrot-01-600x600.jpg?strip=all&lossy=1&ssl=1',
      },
      {
        'id': '4',
        'name': 'Broccoli',
        'description': 'Fresh green broccoli, perfect for healthy meals.',
        'price': 2.5,
        'image': 'https://organicmandya.com/cdn/shop/files/Broccoli.jpg?v=1721371046&width=1000',
      },
      {
        'id': '5',
        'name': 'Milk',
        'description': 'Rich and creamy milk sourced from organic farms.',
        'price': 1.5,
        'image': 'https://images.immediate.co.uk/production/volatile/sites/30/2020/02/Glass-and-bottle-of-milk-fe0997a.jpg',
      },
      {
        'id': '6',
        'name': 'Cheese',
        'description': 'Aged cheese with a rich and tangy flavor.',
        'price': 3.0,
        'image': 'https://blog.wisconsincheeseman.com/wp-content/uploads/sites/10/2022/10/sharp-cheddar-baby-swiss-1-edited-768x697.jpg.webp',
      },
      {
        'id': '7',
        'name': 'Bread',
        'description': 'Soft and fresh bread, baked daily.',
        'price': 2.0,
        'image': 'https://tastesbetterfromscratch.com/wp-content/uploads/2020/05/Oatmeal-Bread24-1.jpg',
      },
      {
        'id': '8',
        'name': 'Eggs',
        'description': 'Farm-fresh eggs packed with protein.',
        'price': 2.2,
        'image': 'https://cdn.britannica.com/94/151894-050-F72A5317/Brown-eggs.jpg',
      },
      {
        'id': '9',
        'name': 'Rice',
        'description': 'Premium quality rice for everyday cooking.',
        'price': 5.0,
        'image': 'https://organicmandya.com/cdn/shop/files/Basmathirice_da3413e4-36cc-4888-884c-f36a3110ead5.jpg?v=1718730887&width=1024',
      },
      {
        'id': '10',
        'name': 'Pasta',
        'description': 'Authentic Italian pasta, ideal for delicious recipes.',
        'price': 3.5,
        'image': 'https://www.biggerbolderbaking.com/wp-content/uploads/2023/09/Penne-Pasta-Thumbnail-scaled.jpg',
      },
    ];


}